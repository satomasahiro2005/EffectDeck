...
struct FiveBandPEQView: View {
    ...
    private var graph: some View {
        FrequencyResponseGraph(
            ...
            onMarkerChanged: { band, hz, db in move(band, hz: hz, db: db) },
    ...
    private var markers: [ETFrequencyMarker] {
        bands.enumerated().map { i, band in
            ETFrequencyMarker(id: i, hz: band.frequency, db: band.gain,
                              label: "\(i + 1)", isActive: band.enabled)
        }
    }
    ...
    /// （five_band_peq.js:1099）。こちらは実際に DSP を回しているレートを見る。
